...
export function getAvailableSources(): SourceEntry[] {
    try {
          ...
          for (const c of chunks) {
                  ...
                  if (!seen.has(key)) {
                            seen.set(key, {
                                        ...
                                        agencyShort: c.agencyShort || "GOV",
                                        year: c.year,
                                        name: `${c.agency} ${c.year}`,
...
export function searchChunks(
    query: string,
    options: { year?: string; agency?: string; country?: string; department?: string; topK?: number } = {}
  ): SearchResult[] {
    const { year, agency, country, department, topK = 8 } = options;
    const chunks = loadChunks();
    const { idf } = buildIdf();
  ...
  const scored = filtered.map((chunk) => {
        ...
        return {
                ...
                country: chunk.country || "Unknown",
                countryCode: chunk.countryCode || "XX",
