...
async function main() {
  ...
  for (const source of PDF_SOURCES) {
      ...
      console.log(`[${source.country} / ${source.agencyShort} ${source.year}] Downloading ${source.name}...`);
        await downloadFile(url, pdfPath);

      console.log(`[${source.country} / ${source.agencyShort} ${source.year}] Extracting text...`);
